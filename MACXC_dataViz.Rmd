...
We can observe that 2018 produced the best team gap out of these four years, while 2019 had the fastest individual 8k time out of any runners in this four year span.
...
Similarly, we can also see which was the "best" for all these metrics the board in this four year span. We can see that 2018 produced the best gap, while 2019 produced the fastest individual time, fastest team pace, and the fastest average team time - all in the same meet.

```{r}
# Best Year for each of the "fastest" Metrics
ALL8K_fast_slow %>%
  filter(Type %in% c("Fastest_IndivTime", "Fastest_TeamTime", "Fastest_pace", "Best_gap")) %>%
  group_by(Type) %>%
  slice(which.min(Result))
```

Conversely, we can see that 2016 produced the slowest average team time, average team pace, and individual time at the Griak meet, and also the worst average gap between runners at the Jim Drews Invitational. All in all, 2016 can be seen as the "worst" Mac Pack team in this four year span.

```{r}
# Worst Year for each of the "slowest" Metrics
ALL8K_fast_slow %>%
  filter(Type %in% c("Slowest_IndivTime", "Slowest_TeamTime", "Slowest_pace", "Worst_gap")) %>%
  group_by(Type) %>%
