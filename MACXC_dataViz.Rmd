...
Then using the list above, I filtered the `MeetALL` data from above based on the list above using `.isin()` and saved this new data frame to an ibject called `seniors`.
...
```{python}
seniors_best = seniors[seniors["Type"] == "8K"]

seniors_best = seniors_best.groupby("LASTNAME").agg(
...
```{python}
seniors_best.set_index("LASTNAME").stack().reset_index()
sns.barplot(x="LASTNAME", y=0, hue= "level_1", data = seniors_best.set_index("LASTNAME").stack().reset_index())
```

```{python}
#Range of times for class 0f 2020 throughout the years
sns.boxplot(x= "LASTNAME", y= "TIME", data = seniors)
```
